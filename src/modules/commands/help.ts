...
export default (self: Self, args: string[]): void => {
    ...
    if(option) loadOption(command as Command, option as unknown as string);
    else if(args[0] && args[0].split("=")[0] === "name" && commands.some((cmd: Command) => cmd.name === args[0].split("=")[1])) {
        ...
        for(const command_1 of commands)
            for(const option of command_1.options ?? [])
                if(!options.includes(option.name)) options.push(option.name);
        console.info(`${green(generateIntro("COMMAND OPTIONS"))} - ${options.map((opt: string) => `${blue(opt)}`).join(", ")}`)
        console.log()
        console.info(yellow(`${generateIntro("COMMANDS LIST")} - Available commands:`))
        for(const command_1 of commands)
            console.info(`${" ".repeat(32 - command_1.name.length)}${yellow(command_1.name)} | ${command_1.description}`)
    }
}