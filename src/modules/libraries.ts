...
export default class LibrariesManager {
    ...
    public isDownloadable(x: Lib | string): boolean {
        return this.libraries.some((lib: Lib) => lib.name === x || lib === x)
    ...
    public isUndownloadable(x: Lib | string): boolean {
        return this.libraries.some((lib: Lib) => lib.name === x)
            && this.downloaded.some((lib: Lib) => lib.name === x);
    }
    ...
    public async download(x: Lib | string): Promise<[Lib[], Lib]> {
        if(!this.isDownloadable(x)) 
            console.warn(`${yellow(generateIntro("ERROR"))} - Oops, it seems like ${yellow(typeof x === "string" ? x : x.name)} cannot be downloaded!`)
    ...
    public async undownload(x: Lib | string): Promise<[Lib[], Lib]> {
        if(!this.isUndownloadable(x)) 
            console.warn(`${yellow(generateIntro("LIBRARY ERROR"))} - Oops, it seems like ${yellow(typeof x === "string" ? x : x.name)} cannot be undownloaded!`)
        this.downloaded.push(this.libraries.find((lib: Lib) => lib.name === x || lib === x)!)
        await this.save()
